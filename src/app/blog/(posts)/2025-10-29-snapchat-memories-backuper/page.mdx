...
The project embraces a service‑oriented structure:

- `cli.py` → Entry point & argument parsing
- `pipeline.py` → Task coordinator
- `executors.py` → Services (ZIP extraction, renaming, combining)
- `download.py` → Parallel download management
- `metadata.py` → EXIF/MP4 metadata handling
- `gpu.py` → Hardware encoder detection

Each piece is modular and replaceable, useful if Snapchat changes its export format again (and they probably will).
